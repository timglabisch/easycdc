...
impl DockerMysql {
    ...
    async fn start_mysql(&mut self) {
        ...
        let docker = docker.args(&[
            ...
            "33069:3306",
            "--mount",
            &format!("type=bind,src={dir}/docker/mysql/my.cnf,dst=/etc/my.cnf"),
    ...
    pub async fn stop_mysql(&mut self) {
        let mut docker = ::tokio::process::Command::new("docker");
        let docker = docker.args(&["kill", &format!("easycdc_integration_{}", self.id)]);
    ...
    pub async fn stop_cdc(&mut self) {
        ...
        for item in res {
            sqlx::query(&format!("KILL {}", item.id))
                .execute(&pool)
                .await
                .unwrap();
        }
